...
async def get_session(
    ...
    """Get an interview session's status."""
...
async def get_session_report(
    ...
    report = await ReportService.build_report(session_id, db)
    if report is None:
        raise HTTPException(status_code=404, detail="Session not found")
    return report
...
async def delete_session(
    ...
    """Delete an interview session and all related records."""
    ...
    # Delete child records first (answers → questions → session)
    from app.models.answer import Answer
    from app.models.question import Question

    await db.execute(
        delete(Answer).where(Answer.session_id == session_id)
    )
    await db.execute(
        delete(Question).where(Question.session_id == session_id)
    )
    await db.flush()
